...
export function upiRetry(outcomes) {
  ...
  do{
    ...
    if(status === "fail" && attempts < 5 && attempts < outcomes.length){
      totalWaitTime += currentWait;
      currentWait *= 2;
    }

  }while(attempts < 5 && attempts < outcomes.length && !isSuccessful);

  return {attempts, success: isSuccessful, totalWaitTime};
}

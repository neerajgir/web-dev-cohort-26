...
export function isValidPincode(code) {
  ...
  const isCorrectLength = code.length === 6;
  const doesNotStartWithZero = !code.startsWith("0");
  const isOnlyDigits = /^\d+$/.test(code);

  return isCorrectLength && doesNotStartWithZero && isOnlyDigits;
}
...
export function formatPostcardField(label, value, width) {
  ...
  const paddingWidth = (typeof width === "number") ? width : 12;
